...
/**
 * Fetch a subcategory by name under a specific parent category
 */
async getSubcategoryByName(
  ...
  try {
    const subcat = await this.prisma.jobCategory.findFirst({
      where: {
        parentId,
        name: {
          equals: name,
    ...
    if (!subcat) {
      return {
        ...
        data: null,
        error: {
          message: 'No subcategory exists with this name under the given parent.',
          details: null,
        },
      };
    }
    ...
    const success_response = {
      ...
      code: "FETCHED",
      category: this.mapCategory(subcat as FullCategory),
      error: null
    }
    ...
    this.logger.error(
      `Failed to fetch subcategory by name: ${err.message}`,
      err.stack,
    );
