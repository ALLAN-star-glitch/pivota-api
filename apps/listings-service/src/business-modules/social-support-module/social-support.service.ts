...
export class SocialSupportService {
  private readonly logger = new Logger(SocialSupportService.name);
  ...
  async createProgram(
    ...
    try {
      const category = await this.prisma.category.findFirst({
        where: { id: dto.categoryId, vertical: 'SOCIAL_SUPPORT' },
      });

      if (!category) {
        return {
          success: false,
          message: 'Invalid category for Social Support',
