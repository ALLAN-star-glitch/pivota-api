...
@Injectable()
export class AuthenticationAnalyticsWorker implements OnModuleInit {
  private readonly logger = new Logger(AuthenticationAnalyticsWorker.name);
  ...
  constructor(
    ...
    console.log('🔥 AuthenticationAnalyticsWorker CONSTRUCTOR called');
  }

  async onModuleInit() {
    console.log('🔥 AuthenticationAnalyticsWorker.onModuleInit() called');
    await this.initialize();
  }

  async initialize() {
    if (this.initialized) {
      console.log('🔥 AuthenticationAnalyticsWorker already initialized, skipping');
      return;
    }
    
    console.log('🔥 AuthenticationAnalyticsWorker.initialize() STARTED');
    const startTime = Date.now();
    this.logger.log('🚀 Initializing authentication analytics worker...');
    ...
    try {
      ...
      this.logger.log(`✅ Authentication analytics worker initialized in ${elapsed}ms`);
      console.log(`🔥 AuthenticationAnalyticsWorker.initialize() COMPLETED SUCCESSFULLY in ${elapsed}ms`);
      
    } catch (error) {
      console.error('🔥 AuthenticationAnalyticsWorker.initialize() FAILED:', error);
