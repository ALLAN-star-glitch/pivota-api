...
export class NotificationsGatewayService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    const jwtSecret = this.configService.get<string>('JWT_SECRET') ?? this.configService.get<string>('NOTIFICATION_SERVICE_JWT_SECRET');
    const atApiKey = this.configService.get<string>('AT_API_KEY');
    const atUsername = this.configService.get<string>('AT_USERNAME');

    if (!jwtSecret) {
      this.logger.error('JWT_SECRET missing in environment');
      throw new Error('JWT_SECRET is required');
    }
    if (!atApiKey || !atUsername) {
      this.logger.error("Africa's Talking API_KEY or USERNAME missing in environment");
      throw new Error("AFRICASTALKING config missing");
    }
  ...
  async sendSms(dto: SendNotificationSmsDto, clientInfo?: NotificationClientInfo) {
    this.logger.debug(`Sending single SMS to: ${dto.to}`);
    try {
  ...
  async getStatus() {
    ...
    }, 'Notification status fetched successfully', 'FETCHED');
  }
